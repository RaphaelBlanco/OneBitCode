function show() {
    // No javascript, através da função show() 
    // que será executada ao clicar no botão, podemos obter 
    // um elemento do DOM através do id:
...
function show() {
    // E o método mais flexível de todos, através de uma query 
    // (semelhante ao que fazemos com o CSS):

    // Obter vários elementos (NodeList) a partir de uma query 
    // (semelhante aos seletores do CSS)
    const contacts = document.querySelectorAll("#contact-list > li > label")
    console.log(contacts)
}

function show() {
    // Além disso, também é possível obter um único elemento 
    // através de uma query. O comportamento é igual ao do 
    // querySelectorAll(), porém selecionando o primeiro elemento 
    // que corresponder à query:
